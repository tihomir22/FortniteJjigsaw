...
public class ChoosePuzzleFromList : MonoBehaviour 
{
	...
	void OnEnable () 
	{
		if (!gameController) 
		{
			Debug.LogWarning("There's no GameController assigned to ChoosePuzzle component");
    ...
    void SwitchPuzzle(int _id)
    {
        if (_id < puzzles.Length  &&  _id >= 0)
        {
            ...
            Debug.LogWarning("Saved puzzle number is out of Puzzles list in ChoosePuzzle component");
