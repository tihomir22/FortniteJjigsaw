...
public class PuzzleImporter_Window : EditorWindow
{
	...
	public static int maxAtlasSize = 4096;	// Max Atlas size
	...
	static Vector2 windowSize = new Vector2(450, 190);
	...
	void OnGUI() 
	{
		...
		GUILayout.BeginVertical("box");
			...
			if (inputFile  &&  oldInputFile != inputFile)
			{
				...
				localDirectory = localDirectory.Substring(localDirectory.IndexOf("/Assets/")+1);
		...
		GUILayout.BeginVertical("box");
			GUILayout.BeginHorizontal();
				GUILayout.BeginVertical();
					...
					maxAtlasSize = Mathf.Clamp (maxAtlasSize, 1024, 4096);
				GUILayout.EndVertical();
		...
		EditorGUILayout.Space();
		material = EditorGUILayout.ObjectField(new GUIContent("Use custom material:", "This Material will be used for generated puzzle-elements") ,material, typeof(Material), false) as Material;	
		GUILayout.EndVertical();
		...
		if(inputFile)  
		{
			...
			{ 
				...
				CreateAtlas ();
				ConvertToSprites ();
				CreateGameObjects().AddComponent<PuzzleController>().Prepare();
	...
	{ 
		...
		for (int i = 0; i < textureArray.Length; i++)   
		{
			...
			EditorUtility.DisplayProgressBar( "Step 1/4",	"Compose atlas...", i/(textureArray.Length-1.1f));
		}
	...
	{ 	
		...
		for (int i = 0; i < sprites.Length; i++) 
		{
			...
			EditorUtility.DisplayProgressBar( "Step 2/4", "Convert textures to Sprites...", i/(sprites.Length-1.1f));
		}
	...
	GameObject CreateGameObjects ()  
	{
		...
		for (int i = 0; i < layers.Length; i++)
		{    
			EditorUtility.DisplayProgressBar( "Step 3/4", "Create GameObjects...", i/(layers.Length-1.1f));
